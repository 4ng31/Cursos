...
if nargin < 2,
    ...
    switch caso
        ...
        case 2,
            ...
            X1=[rho'.*cos(theta')+4*rand(N,1) rho'.*sin(theta')+5*rand(N,1)];
            X2=[-rho'.*cos(theta')+3*rand(N,1) -rho'.*sin(theta')+6*rand(N,1)];
            Y1=ones(N,1);   %labels of class 1
        ...
        case 3 
            ...
            X2=1.5*randn(N,d)+2.75*ones(N,1)*(1*ones(1,d));    %generation of the fist class data set
            Y1=ones(N,1);   %labels of class 1
            Y2=-ones(N,1);  %labels of class 2
            X=[X1;X2];
            Y=[Y1;Y2];
